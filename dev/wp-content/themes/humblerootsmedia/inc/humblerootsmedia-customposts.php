...
function add_testimonial_metaboxes() {
    ...
    add_meta_box('testimonial_company', 'Testimonial Company', 'testimonial_company_display', 'testimonial', 'normal', 'default');
...
function testimonial_company_display() {
  global $post;
  ...
  // Get the location data if its already been entered
  $location = get_post_meta($post->ID, 'testimonial_company', true);

  // Echo out the field
  echo '<input type="text" name="testimonial_company" value="' . $location  . '" class="widefat" />';
}
...
function testimonial_save_data($post_id, $post) {
  ...
  $events_meta['testimonial_company'] = $_POST['testimonial_company'];
